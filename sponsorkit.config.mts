...
import { tagged as svg } from 'foxts/tagged';
...
const VERCEL_LOGO_SCALE = 0.7;
const VERCEL_LOGO_WIDTH = Math.round(307 * VERCEL_LOGO_SCALE);
const VERCEL_LOGO_HEIGHT = Math.round(82 * VERCEL_LOGO_SCALE);

function VercelLogo(width: number, y: number) {
  return svg`
<a xlink:href="https://vercel.com" class="sponsorkit-link" target="_blank" id="Vercel">
  <svg width="${VERCEL_LOGO_WIDTH}" height="${VERCEL_LOGO_HEIGHT}" viewBox="0 0 307 82" x="${(width - VERCEL_LOGO_WIDTH) / 2}" y="${y}" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      composeAfter(compose, _, config) {
        if (config.filter?.({ monthlyDollars: Infinity } as any, []) !== false) {
          compose
            ...
            .addSpan(10)
            .addRaw(VercelLogo(config.width!, compose.height))
            .addSpan(VERCEL_LOGO_HEIGHT + 18);
        }
      }
    }
  ],
